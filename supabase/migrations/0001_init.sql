...
-- Helper: is the current user a member of a trip (any role)?
...
create policy "members insert self via invite or owner"
  ...
  with check (
    ...
    -- Or the user is adding themselves (invite acceptance flow inserts as the invitee)
    or user_id = auth.uid()
  );
...
drop policy if exists "days mutate by editor" on public.trip_days;
...
-- Realtime
-- =========================================================================
-- Add tables to the realtime publication so collaborators see live updates.
do $$
begin
  if not exists (
